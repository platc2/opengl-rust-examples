...
/*
        ...
        if link_successful {
            if let Some(info_log) = info_log {
                println!("Program linked successfully: {info_log}");
            }
            Ok(Self { id })
        } else {
            ...
            Err(ProgramLink(
                info_log.unwrap_or(String::from("Unknown error")),
            ))
        }
*/
        unimplemented!()
...
*/
    ...
    #[must_use]
    pub const fn id(&self) -> gl::ProgramId {
        self.id
    }
}
