...
impl Application for State {
    fn tick(&mut self, time: &Time<Instant>, input_manager: &dyn InputManager) {
        ...
        // Directional light
        gl::uniform_3fv(
            gl::uniform_location(self.cube_program, "dirLight.direction"),
            glm::value_ptr(&glm::vec3(-0.2, -1., -0.3)),
        );
        gl::uniform_3fv(
            gl::uniform_location(self.cube_program, "dirLight.ambient"),
            glm::value_ptr(&glm::vec3(0.05, 0.05, 0.05)),
        );
        gl::uniform_3fv(
            gl::uniform_location(self.cube_program, "dirLight.diffuse"),
            glm::value_ptr(&glm::vec3(0.4, 0.4, 0.4)),
        );
        gl::uniform_3fv(
            gl::uniform_location(self.cube_program, "dirLight.specular"),
        ...
        for (index, point_light) in self.point_lights.iter().enumerate() {
            gl::uniform_3fv(
                gl::uniform_location(self.cube_program, format!("pointLights[{index}].position")),
                glm::value_ptr(point_light),
            );
            gl::uniform_3fv(
                gl::uniform_location(self.cube_program, format!("pointLights[{index}].ambient")),
                glm::value_ptr(&glm::vec3(0.05, 0.05, 0.05)),
            );
            gl::uniform_3fv(
                gl::uniform_location(self.cube_program, format!("pointLights[{index}].specular")),
                glm::value_ptr(&glm::vec3(0.8, 0.8, 0.8)),
            );
            gl::uniform_3fv(
                gl::uniform_location(self.cube_program, format!("pointLights[{index}].diffuse")),
                glm::value_ptr(&glm::vec3(1., 1., 1.)),
            );
            gl::uniform_1f(
                gl::uniform_location(self.cube_program, format!("pointLights[{index}].constant")),
                1.,
            );
            gl::uniform_1f(
                gl::uniform_location(self.cube_program, format!("pointLights[{index}].linear")),
                0.09,
            );
            gl::uniform_1f(
                gl::uniform_location(self.cube_program, format!("pointLights[{index}].quadratic")),
                0.032,
            );
        }
        ...
        // Spot light
        gl::uniform_3fv(
            gl::uniform_location(self.cube_program, "spotLight.position"),
            glm::value_ptr(self.camera.position()),
        );
        gl::uniform_3fv(
            gl::uniform_location(self.cube_program, "spotLight.direction"),
            glm::value_ptr(self.camera.front()),
        );
        gl::uniform_3fv(
            gl::uniform_location(self.cube_program, "spotLight.ambient"),
            glm::value_ptr(&glm::vec3(0., 0., 0.)),
        );
        gl::uniform_3fv(
            gl::uniform_location(self.cube_program, "spotLight.diffuse"),
            glm::value_ptr(&glm::vec3(1., 1., 1.)),
        );
        gl::uniform_3fv(
            gl::uniform_location(self.cube_program, "spotLight.specular"),
            glm::value_ptr(&glm::vec3(1., 1., 1.)),
        );
        gl::uniform_1f(
            gl::uniform_location(self.cube_program, "spotLight.constant"),
            1.,
        );
        gl::uniform_1f(
            gl::uniform_location(self.cube_program, "spotLight.linear"),
            0.09,
        );
        gl::uniform_1f(
            gl::uniform_location(self.cube_program, "spotLight.quadratic"),
            0.032,
        );
        gl::uniform_1f(
            gl::uniform_location(self.cube_program, "spotLight.cutoff"),
            12.5f32.to_radians().cos(),
        );
        gl::uniform_1f(
            gl::uniform_location(self.cube_program, "spotLight.outerCutoff"),
            15f32.to_radians().cos(),
        );
...
impl App for State {
        ...
        Self: Sized,
    {
        ...
        Ok(Self {
            ...
            views: vec![Box::new(SettingsView::new(shininess.clone()))],
        })
    }
}
