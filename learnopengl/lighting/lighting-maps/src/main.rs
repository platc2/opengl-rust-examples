...
pub fn main() -> Result<()> {
    ...
    let emission_texture = gl::create_texture(gl::TextureTarget::TEXTURE_2D);
    gl::bind_texture(gl::TextureTarget::TEXTURE_2D, emission_texture);

    let texture_bytes = include_bytes!("../assets/container2_emission.png");
    unsafe {
        // Hack because the wrapper library does not support setting this parameter (yet)
        stb_image::stb_image::stbi_set_flip_vertically_on_load(1);
    }
